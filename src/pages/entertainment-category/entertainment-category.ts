...
@IonicPage()
@Component({
  selector: 'page-entertainment-category',
  templateUrl: 'entertainment-category.html',
})
export class EntertainmentCategoryPage {
  ...
  ionViewWillEnter() {
    ...
    //get top headlines under the entertainment category
    this.getTopHeadLinesEntertainment();
  }
  ...
  */
  ...
  getTopHeadLinesEntertainment(): void {

    //initiate loader here
    // this.news_provider.showLoader("Loading top headlines...");
    let loadingCtrl = this.loader.create({ content: "Loading entertainment headlines..." });
    loadingCtrl.present();
    this.news_provider.getTopHeadlinesEntertainment()
      .subscribe(data => {
        ...
        if (data) {
          ...
          //check if there were news podcasts from response
          if (total_feeds > 0) {


            //clean and filter data to be rendered on html
